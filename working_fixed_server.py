...
class WorkingMCPServer:
    def __init__(self, services: Optional[str] = None):
        ...
        sys.stderr.write("Working MCP Server starting...\n")
        sys.stderr.flush()
    ...
    async def _load_tools_async(self):
        ...
        try:
            # 配置服务列表
            if self.services:
                service_keys = [s.strip().lower() for s in self.services.split(",")]
            ...
            # 获取工具字典
            tools_dict = await self.fastmcp_instance.get_tools()
    ...
    async def run(self):
        """运行服务器"""
        sys.stderr.write("Working MCP Server ready\n")
        sys.stderr.flush()
...
def main():
    parser = argparse.ArgumentParser(description="Working Alibaba Cloud MCP Server")
    ...
    server = WorkingMCPServer(services=args.services)
